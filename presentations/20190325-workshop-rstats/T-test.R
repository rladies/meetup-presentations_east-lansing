...
# Load and attach the sleep dataframe
data(sleep)
attach (sleep)
...
ggplot(sleep, aes(x=extra, color=group, fill=group)) +
  geom_histogram(aes(y=..density..), binwidth = 0.5, position="identity", alpha=0.2)+   # Histogram with density instead of count on y-axis
  geom_density(alpha=0.6)+
#  geom_vline(data=mu, aes(xintercept=grp.mean, color=group),
 #            linetype="dashed")+
  scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"))+
  scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"))+
  labs(title="Sleep histogram plot",x="Extra Sleep (Hrs.)", y = "Density")+
  theme_classic()
...
pwr.t.test(n= len, d = effect_size, sig.level = 0.05 , type = "two.sample", alternative = "two.sided")
